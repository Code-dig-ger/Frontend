...
function ProblemsPage({ info, queryStr }) {
  ...
  const [displayDiff, setDisplayDiff] = useState(
    queryDefault.difficulty
      ? {
          values: [
            queryDefault.difficulty.includes('B'),
            queryDefault.difficulty.includes('E'),
            queryDefault.difficulty.includes('M'),
            queryDefault.difficulty.includes('H'),
            queryDefault.difficulty.includes('S'),
            queryDefault.difficulty.includes('C'),
          ],
        }
  ...
  const [displayTags, setDisplayTags] = useState(
    queryDefault.tags
      ? {
          values: [
            queryDefault.tags.includes('string'),
            queryDefault.tags.includes('dp'),
            queryDefault.tags.includes('math'),
            queryDefault.tags.includes('combinatorics'),
            queryDefault.tags.includes('Number Theory'),
            queryDefault.tags.includes('interactive'),
            queryDefault.tags.includes('Binary Search'),
            queryDefault.tags.includes('greedy'),
            queryDefault.tags.includes('graph'),
          ],
        }
      : {
          values: [
  ...
  const platformFilters = ['C', 'F', 'A', 'S', 'U']

  const difficultyFilters = ['B', 'E', 'M', 'H', 'S', 'C']
  ...
  const [platformQueries, setPlatformQueries] = useState(
    queryDefault.platform ? queryDefault.platform.split(',') : []
  )
  const [difficultyQueries, setDifficultyQueries] = useState(
    queryDefault.difficulty ? queryDefault.difficulty.split(',') : []
  )
  const [tagQueries, setTagQueries] = useState(
    queryDefault.tags ? queryDefault.tags.split(',') : []
  )
  // var difficultyQueries=[];
  // var TagQueries=[];

  const [diffRange, setDiffRange] = useState(
    queryDefault.range_l && queryDefault.range_r
      ? [queryDefault.range_l, queryDefault.range_r]
      : queryDefault.range_l
      ? [queryDefault.range_l, 3200]
      : queryDefault.range_r
      ? [0, queryDefault.range_r]
      : [100, 3200]
  )
  const [sliderChange, setSliderChange] = useState(
    queryDefault.range_l || queryDefault.range_r ? true : false
  )
  ...
  const changePlatformFilter = (event, lev) => {
    ...
    if (res) {
      ...
      const newList = platformQueries.filter(
        (item) => item != platformFilters[lev]
      )
  ...
  return show == true ? (
    ...
    <>
      ...
      <div id="mySidenav" className="sidenav">
        ...
        <Button
          className="filterHeading"
          onClick={(e) => setOpenTags(!openTags)}
        >
          Tags
        </Button>
        <Modal
                ...
                {defaultTags.map((lev, i) => {
                  if (displayTags.values[i]) {
                    return (
                      <Form.Check
                        checked={true}
                        onChange={(event) => changeTagFilter(event, i)}
                        inline
                        label={lev}
                        type="checkbox"
                        id={`inline-${lev}-${i}`}
        ...
        <Button
          className="filterHeading"
          onClick={(e) => setModalOpenPlat(!modalOpenPlat)}
        >
          Platforms
        </Button>
        <Modal
          toggle={(e) => {
            setModalOpenPlat(false)
          }}
          isOpen={modalOpenPlat}
        >
          <ModalBody>
                ...
                {platforms.map((lev, i) => {
                  ...
                  if (displayPlat.values[i]) {
                    return (
                      <Form.Check
                        checked={true}
                        onChange={(event) => changePlatformFilter(event, i)}
                        inline
                        label={lev}
                        type="checkbox"
                        id={`inline-${lev}-${i}`}
        ...
        <Button
          className="filterHeading"
          onClick={(e) => setModalOpenDiffiRange(!modalOpenDiffiRange)}
        >
          Difficulty Range
        </Button>
        <Modal
          toggle={(e) => {
            setModalOpenDiffiRange(false)
          }}
          isOpen={modalOpenDiffiRange}
        >
          <ModalBody>
            {/* <Form inline>
