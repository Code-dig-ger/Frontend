...
function ProblemsPage({info,queryStr}) {
    ...
    const changePlatformFilter = (event,lev) => {
        // console.log(queryString.stringifyUrl({url: 'https://143.110.253.225/problems/', query: {platform: 'F,A',difficulty:'B,E'}}));
    ...
    function addProblem(slug){
        ...
        // console.log(slug, prob_id, platform)
          const result =  fetch (`https://143.110.253.225/lists/userlist/add`,{
              method:"POST",
              headers:{
    ...
    async function getPlaylists()
  {
    ...
    const res = await fetch(`https://143.110.253.225/lists/userlist/`, {
            method:"GET",
            headers:{
                "Content-Type":"application/json",
                "Authorization":`Bearer ${creds.access}`
